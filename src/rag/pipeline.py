...
from ..memory.manager import MemoryManager
from ..utils.metrics import record_time
from ..utils.acronyms import expand_query_text, acronym_signal_boost  # <<< NEW
...
def _topic_overlap(question: str, ch: dict) -> int:
    """
    Build a lightweight lexical score using the *expanded* query so acronyms
    (CSRF/XSS/SSRF/SQLi/IDOR/XXE/…) and their long forms contribute.
    """
    qx = expand_query_text(question).lower()
    title_url = (ch.get("title", "") + " " + ch.get("url", "")).lower()
    text = (ch.get("text", "") or "").lower()[:1200]

    # tokens + include hyphen/space variants
    base_terms = [t for t in re.findall(r"[a-z0-9\-]+", qx) if len(t) > 2]
...
class RAGPipeline:
    ...
    def _dense_candidates(self, q_text: str, k: int) -> List[Tuple[float, Dict]]:
        """
        Acronym-aware dense retrieval:
        - expand query (e.g., "CSRF" => "... cross-site request forgery ...")
        - pull a larger pool (caller passes candidate-multiplied k)
        - apply a small content-based boost for expanded terms
        """
        qx = expand_query_text(q_text)
        # ask the store for dense candidates; if your FaissStore.search_text already boosts, that's ok.
        dense = self.store.search_text(qx, embedder=self.embedder, top_k=k, boost=False)
        # apply a tiny, general signal boost if title/text mention expansion terms
        hits = [{"score": float(s), **m} for s, m in dense]
        hits = acronym_signal_boost(q_text, hits, alpha=0.15)
        hits.sort(key=lambda h: h.get("score", 0.0), reverse=True)
        hits = hits[:k]
        return [(h["score"], {kk: vv for kk, vv in h.items() if kk != "score"}) for h in hits]
    ...
    def retrieve(self, question: str, k: int | None = None, diversify: bool | None = None):
        ...
        if diversify is None:
            diversify = True
        # expand acronyms/synonyms before retrieval
        q_text = expand_query_text(question)
